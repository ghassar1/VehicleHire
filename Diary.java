 ...
 * @version April 2013
 */
public  class  Diary //implements Reservation

{
   ...
   public void addReservation(VehicleReservation reservation)
   {
     Date date = reservation.getStartDate();
      for(int day=1; day<=reservation.getNoOfDays(); day++)
      {
         ...
         dayInDiaryMap.get(date).addEntry(reservation, day);
    ...
    * @parameter     vehicleReservation, of type VehicleReservation 
    */   
   public void deleteReservation(VehicleReservation reservation)
   {
      Date date = reservation.getStartDate();
      for(int day=1; day<=reservation.getNoOfDays(); day++)
      {
         DayInDiary dayInDiary = dayInDiaryMap.get(date);
         dayInDiary.deleteEntry(reservation);
   ...
   private class DayInDiary
   {
      ...
      private VehicleReservation[] getReservations()
      {
         ...
         VehicleReservation[] reservation = new VehicleReservation[noOfEntries];
         for(int i=0; i<noOfEntries; i++)
            reservation[i] = daysEntries.get(i).getReservation();
         return reservation;
      }
    
      private void addEntry(VehicleReservation reservation, int dayNo)
      {
         daysEntries.add(new Entry(reservation, dayNo));
      }
   
      private void deleteEntry(VehicleReservation reservation)
      {
         // find the entry for this reservation and delete it
         Entry toBeDeletedEntry = null;
         for(Entry entry : daysEntries)
         {
            if( entry.getReservation()==reservation )  // in this situation, this is better than using "equals()"
            {
      ...
      private void printEntries()
      {
         ...
         if( size>0 )
         {
            ...
            for( Entry entry: daysEntries )
            {
               VehicleReservation reservation = entry.getReservation();
               int reservationDay = DateUtil.daysBetween(reservation.getStartDate(), date) + 1;;
               System.out.println("    " + reservation + ", day " + reservationDay + " of " + reservation.getNoOfDays());
            }
         }
         else
         {
      ...
      private class Entry
      {
         ...
         private Entry(VehicleReservation reservation,  int reservationDay)
         {
